...
def search_alive_hosts(ip, directory, isList, tcp_ports, udp_ports):
    ...
    portlistUDP = "53,67,69,88,123,137,138,161,162,389,445,464,514,1812,1813,3544"

    if tcp_ports :
        portlistTCP = portlistTCP + "," +  tcp_ports
    if udp_ports : 
        portlistUDP = portlistUDP + "," + udp_ports

    if isList :
        cmd = ["sudo", "nmap", "-sT", "-Pn", "-p", portlistTCP, "-iL", ip, "-oA", directory + "/nmap/alive_hosts_tcp"]
        run_command(cmd)

        cmd = ["sudo", "nmap", "-sU", "-Pn", "-p", portlistUDP, "-iL", ip, "-oA", directory + "/nmap/alive_hosts_udp"]
        run_command(cmd)
    else :
        cmd = ["sudo", "nmap", "-sT", "-Pn", "-p", portlistTCP, ip, "-oA", directory + "/nmap/alive_hosts_tcp"]
        run_command(cmd)

        cmd = ["sudo", "nmap", "-sU", "-Pn", "-p", portlistUDP, ip, "-oA", directory + "/nmap/alive_hosts_udp"]
        run_command(cmd)

def extract_alive_hosts(directory, openvasTCP, openvasUDP):
    if os.path.isfile(directory + "/tcp_scan.txt") :
        cmd = ["rm", directory + "/tcp_scan.txt"]
        run_command(cmd)
    cmd = ["touch", directory + "/tcp_scan.txt"]
    run_command(cmd)
    if os.path.isfile(directory + "/udp_scan.txt") :
        cmd = ["rm", directory + "/udp_scan.txt"]
        run_command(cmd)
    cmd = ["touch", directory + "/udp_scan.txt"]
    run_command(cmd)
    ...
    try:
        treeUDP = elementTree.parse(directory + "/nmap/alive_hosts_udp.xml")
    except elementTree.ParseError as e:
        print("Compare elementTree.ParseError: ", e)
        cleanedData = clean_xml_data(directory + "/nmap/alive_hosts_udp.xml")
        treeUDP = elementTree.parse(io.StringIO(cleanedData))
        pass
    aliveHostsUDP = []
    tempHost = None

    rootUDP = treeUDP.getroot()
    for child in rootUDP.findall("host"):
        up = False
        for host in child.findall("address"):
            if host.attrib['addrtype'] == 'ipv4':
                tempHost = str(host.attrib['addr'])
        ...
        if up == True :
            for ports in child.findall('ports') :
                for port in ports.findall('port'):
                    if port.find('state').attrib['state'] == 'open' :
                        if tempHost not in aliveHostsUDP :
                            aliveHostsUDP.append(tempHost)

    outfile = open(directory + "/udp_scan.txt", "at")
    if aliveHostsUDP:
        lastItem = aliveHostsUDP[-1]
        for host in aliveHostsUDP:
            outfile.write(host + "\n")
            openvasUDP.write(host)
            if host == lastItem :
                pass
            else :
                openvasUDP.write(',')
    ...
    if ip:
        if skip:
            if isList:
                ...
                cmd = ["sudo", "nmap", "-sU", "-sV", "-Pn", "--top-ports", "1000", "-iL", ip, "-oA", directory + "/nmap/versionscan_udp"]
                run_command(cmd)
            else:
                print("Starting the versionscan on the alive hosts\n")
                ...
                cmd = ["sudo", "nmap", "-sU", "-sV", "-Pn", "--top-ports", "1000", ip, "-oA", directory + "/nmap/versionscan_udp"]
                run_command(cmd)
    else:
        print("Starting the versionscan on the alive hosts\n")
        ...
        cmd = ["sudo", "nmap", "-sU", "-sV", "-Pn", "--top-ports", "1000", "-iL", directory + "/udp_scan.txt", "-oA", directory + "/nmap/versionscan_udp"]
        run_command(cmd)

def extract_information(directory, openvasTCP, openvasUDP, skip = False) :
    ...
    try:
        treeUDP = elementTree.parse(directory + "/nmap/versionscan_udp.xml")
    except elementTree.ParseError as e:
        print("Compare elementTree.ParseError: ", e)
        cleanedData = clean_xml_data(directory + "/nmap/versionscan_udp.xml")
        treeUDP = elementTree.parse(io.StringIO(cleanedData))
        pass
    rootUDP = treeUDP.getroot()
    portOpenvasUDP = []
    ...
    for child in rootUDP.findall("host") :
        ...
        for ports in child.findall('ports') :
            for port in ports.findall('port') :
                if port.find('state').attrib['state'] == 'open' :
                    if skip:
                        hostOpenvasUDP.append(str(tempHost))
                    if str(port.attrib['portid']) not in portOpenvasUDP:
                        portOpenvasUDP.append(str(port.attrib['portid']))
                    overview[tempHost]["U" + str(port.attrib['portid'])] = {
                        'protocol': str(port.attrib['protocol']),
                        'state': str(port.find('state').attrib['state']) if 'state' in port.find('state').attrib else "no_state",
                        'name': str(port.find('service').attrib['name']) if 'name' in port.find('service').attrib else "no_name",
                        'product': str(port.find('service').attrib['product']) if 'product' in port.find('service').attrib else "no_product",
    ...
    for key in overview : 
        ...
        for port in overview[key] :
            ...
            outfile.write("\t" if len(overview[key][port]['name']) > 7 else "\t\t")
    ...
    if skip:
        if hostOpenvasUDP:
            last = hostOpenvasUDP[-1]
            for host in hostOpenvasUDP:
                openvasUDP.write(host)
                if host == last:
                    pass
                else:
    ...
    if portOpenvasUDP :
        last = portOpenvasUDP[-1]
        for port in portOpenvasUDP:
            openvasUDP.write(port)
            if port == last:
                pass
            else:
                openvasUDP.write(',')
...
def compare(overview, oldResult, directory):
    ...
    if os.path.isfile(directory + "/nmap/versionscan_udp.xml"):
        try:
            treeUDP = elementTree.parse(directory + "/nmap/versionscan_udp.xml")
        except elementTree.ParseError as e:
            print("Compare elementTree.ParseError: ", e)
        ...
        for child in rootUDP.findall("host") :
            ...
            for ports in child.findall('ports') :
                for port in ports.findall('port') :
                    if port.find('state').attrib['state'] == 'open' :
                        oldOverview[tempHost]["U" + str(port.attrib['portid'])] = {
                            'protocol': str(port.attrib['protocol']),
                            'state': str(port.find('state').attrib['state']) if 'state' in port.find('state').attrib else "no_state",
                            'name': str(port.find('service').attrib['name']) if 'name' in port.find('service').attrib else "no_name",
                            'product': str(port.find('service').attrib['product']) if 'product' in port.find('service').attrib else "no_product",
...
def main():
    ...
    parser.add_argument("-up", "--udp-ports", dest="udp_ports", help="Add some extra and exotic udp ports, which should fulfill the host discovery part")
    parser.set_defaults(udp_ports="")
    ...
    if subfolder and not folder:
        ...
        run_command(cmd)
        cmd = ["touch", directory + "/openvas_preparation_udp.txt"]
        run_command(cmd)
        
        openvasTCP = open(directory + "/openvas_preparation_tcp.txt", "w")
        openvasUDP = open(directory + "/openvas_preparation_udp.txt", "w")

        if skip:
            the_real_scan(directory, isList, skip, ip)
        else:
            search_alive_hosts(ip, directory, isList, tcp_ports, udp_ports)
            extract_alive_hosts(directory, openvasTCP, openvasUDP)
            the_real_scan(directory)

        overview = extract_information(directory, openvasTCP, openvasUDP, skip)
